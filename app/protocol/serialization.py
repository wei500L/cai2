...
from app.protocol.outgoing import BaseEnvelope


def serialize_json(envelope: BaseModel) -> bytes:
    return envelope.model_dump_json().encode()


def deserialize_json(raw: bytes, payload_model: type[BaseModel]) -> BaseModel:
    if issubclass(payload_model, BaseEnvelope):
        return payload_model.model_validate_json(raw)
