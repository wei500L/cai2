...
| resolve.map_diff | `ResolveMapDiffMessage.p`: `room_id`, `epoch`, `turn`, `changes`, `border_updates` | `ResolveMapDiffPayload`: `room_id`, `epoch`, `turn`, `changes`, `border_updates` | 无；后端 diff item 以 `dict[str, Any]` 承载，`border_updates` 现在携带 `between/tension/visual_state` | 是 |
